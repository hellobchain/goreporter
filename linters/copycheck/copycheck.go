...
import (
	...
	"github.com/hellobchain/goreporter/linters/copycheck/job"
	"github.com/hellobchain/goreporter/linters/copycheck/output"
...
func CopyCheck(projectPath string, except string) (result [][]string) {
	flag.Parse()
	if html && plumbing {
		logger.Error("you can have either plumbing or HTML output")
		return result
	}
	paths := []string{projectPath}
	if verbose {
		logger.Error("Building suffix tree")
	}
	...
	if verbose {
		logger.Error("Searching for clones")
	}
