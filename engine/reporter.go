...
import (
	...
	"github.com/hellobchain/goreporter/utils"
)
...
func (r *Reporter) Report() error {
	defer r.Close()
	logger.Info("start code quality assessment...")
...
func (r *Reporter) compute(strategy StrategyLinter, params StrategyParameter) {
	logger.Infof("running %s...", strategy.GetName())
	...
	logger.Infof("%s over!", strategy.GetName())
}
...
func (r *Reporter) Render() (err error) {
	...
	case "text":
		err = r.toText()
	default:
		logger.Infof(fmt.Sprintf("Generating HTML report,time consuming %vs", time.Since(r.StartTime).Seconds()))
		err = r.toHtml()
		if err != nil {
			logger.Error("Json2Html error:", err)
			return
		}
	}
	return
...
func (r *Reporter) toJson() (err error) {
	logger.Infof(fmt.Sprintf("Generating json report,time consuming %vs", time.Since(r.StartTime).Seconds()))
	...
	logger.Info("Json report saved in:", jsonpath)
	return
}

func (r *Reporter) toText() (err error) {
	logger.Infof(fmt.Sprintf("Generating text report,time consuming %vs", time.Since(r.StartTime).Seconds()))
	color.Magenta(
		headerTpl,
		r.Project,
		int(r.GetFinalScore()),
